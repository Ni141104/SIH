...
const ResponseDisplay = ({ responseText }) => {
  ...
  return (
    <div className="w-screen min-h-screen bg-gray-100 flex items-center justify-start">
    <div className="mx-7  bg-white p-8 rounded-lg shadow-md w-full max-w-md mb-6">
      <h1 className="text-2xl font-bold text-center mb-6 text-black">Extracted text</h1>
      <div className="p-4 bg-gray-200 rounded-lg overflow-y-auto max-h-[400px]">
        <p className="text-gray-700 break-words">{responseText}</p>
      </div>
    </div>
  
    <div className="mx-7 bg-white p-8 rounded-lg shadow-md w-full max-w-md mb-6">
      <h1 className="text-2xl font-bold text-center mb-6 text-black">SHA256 Hash pushed on the blockchain</h1>
      <div className="p-4 bg-gray-200 rounded-lg overflow-y-auto max-h-40">
        <p className="text-gray-700 break-words">{sha256(responseText)}</p>
      </div>
    </div>
  
    <div className="mx-7 bg-white p-8 rounded-lg shadow-md w-full max-w-md mb-6">
      <h1 className="text-2xl font-bold text-center mb-6 text-black">Ed25519 Signature</h1>
      <div className="p-4 bg-gray-200 rounded-lg overflow-y-auto max-h-40">
        <p className="text-gray-700 break-words">{signature.length > 0 ? signature : "Generating signature..."}</p>
      </div>
      ...
      {signature && (
        <div className="mt-6 flex justify-center">
          <div>
            <h2 className="text-xl font-bold text-center mb-4 text-black">QR Code</h2>
            <QRCodeCanvas value={signature} size={256} level="L" includeMargin={true} />
          </div>
        </div>
      )}
    </div>
